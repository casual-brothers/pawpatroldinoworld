...
if __name__ == "__main__":
	...
	std_output = open('check'+ '.log', mode="w", encoding='utf-16')
	...
	for entry in json_man['Children']:
		...
		if mansize > 1:
			...
			std_output.write('duplicates ' +key + ' ' + str(mansize) +' '+ source + '\n')
			for k in entry['Keys']:
			...
			if (source_key not in man_entries):
				...
				if source_key != "-WIP-":
					if source_key not in duplicates:
						...
						text1 = source_key.encode('utf-8').decode('ascii', 'ignore')
						text2 = source.encode('utf-8').decode('ascii', 'ignore')
						std_output.write('not found in manifest: ' + source_key + ' ' + source +'\n')
						try:
							print('not found in manifest: ' + source_key + ' ' + source)
						except UnicodeEncodeError:
							print('not found in manifest: ' + text1 + ' ' + text2)
	...
	for source_key in double_key_in_excel_print:
		if source_key not in valid_double_keys:
			...
			std_output.write('duplicated key in excel [' + source_key + ']' +'\n')
		...
		if bool(source_key and source_key.strip()) and bool(source and source.strip()):
			if source in man_sources:
				...
				if expected_key!=source_key:
					...
					std_output.write('key mismatch manifest excel: ' + source_key + ' ' + expected_key + ' ' + source+'\n')
					key_mismatch += 1
			...
			if source_key in man_entries:
				...
				if source != sourcein:
					...
					if escaped_sourcein != source:		
						...
						std_output.write('different source: ' +'['+ source_key + '][' + escaped_sourcein + '][' + source +']'+'\n')
						source_mismatch += 1
						outsource = sourcein
	...
	for row in json_content[SHEET_NAME]:
		...
		if not bool(source_key and source_key.strip()) and bool(source and source.strip()):
			...
			std_output.write('missing key in excel: ' + '[' + candidatekey +']'+ '[' + source +']'+'\n')
			missing_keys += 1
	...
	for entry in man_entries:
		...
		std_output.write('leftover: ' +'['+ entry + '][' + man_entries[entry] + ']'+'\n')
		leftovers += 1
