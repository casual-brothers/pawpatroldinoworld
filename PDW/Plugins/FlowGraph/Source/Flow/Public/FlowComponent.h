...
public:
	...
	// Send single notification from the actor to owner Flow graph
	// If set on server, it's always going to be replicated to clients
	UFUNCTION(BlueprintCallable, Category = "Flow")
	void NotifyOwnerGraph(const FGameplayTag NotifyTag, const EFlowNetMode NetMode = EFlowNetMode::Authority);
...
public:
	...
	FFlowComponentNotify OnNotifyFromSelf;
...
public:
	...
	virtual void NotifyFromOwnerGraph(const FGameplayTagContainer& NotifyTags, const EFlowNetMode NetMode  = EFlowNetMode::Authority);
private:
	UFUNCTION()
...
public:
	...
	UPROPERTY(BlueprintAssignable, Category = "Flow")
	FFlowComponentDynamicNotify SelfNotify;
...
public:
	...
	// If true, component will load Root Flow on Begin Play
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "RootFlow")
	bool bAutoLoadRootFlow = true;
