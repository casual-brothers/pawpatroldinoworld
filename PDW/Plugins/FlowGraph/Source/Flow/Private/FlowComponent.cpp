...
void UFlowComponent::BeginRootFlow(bool bComponentLoadedFromSaveGame)
{
	if (RootFlow)
	{
		if (bComponentLoadedFromSaveGame && bAutoLoadRootFlow)
		{
			LoadRootFlow();
		}
...
void UFlowComponent::NotifyOwnerGraph(const FGameplayTag NotifyTag, const EFlowNetMode NetMode /* = EFlowNetMode::Authority*/)
{
	if (IsFlowNetMode(NetMode) && NotifyTag.IsValid() && HasBegunPlay())
	{
		// save recently notify, this allows for the retroactive check in nodes
...
#endif
		OnNotifyFromSelf.Broadcast(this,NotifyTag);
	}
}
...
void UFlowComponent::NotifyFromOwnerGraph(const FGameplayTagContainer& NotifyTags, const EFlowNetMode NetMode /* = EFlowNetMode::Authority*/)
{
	if (IsFlowNetMode(NetMode) && NotifyTags.IsValid() && HasBegunPlay())
	{
		FGameplayTagContainer ValidatedTags;
		...
		if (ValidatedTags.Num() > 0)
		{
			for (const FGameplayTag& ValidatedTag : ValidatedTags)
			{
				SelfNotify.Broadcast(nullptr, ValidatedTag);
			}
