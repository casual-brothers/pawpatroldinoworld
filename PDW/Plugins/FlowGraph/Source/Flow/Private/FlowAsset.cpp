...
void UFlowAsset::TriggerJump(const FName& EventName, IFlowDataPinValueSupplierInterface* DataPinValueSupplier)
{
	for (UFlowNode_CustomInput* CustomInputNode : CustomInputNodes)
	{
		if (CustomInputNode->EventName == EventName)
		{
			...
			CustomInputNode->ActivationState = EFlowNodeState::Active;
			CustomInputNode->TriggerFirstOutput(true);
		}
	}
}
