...
FFlowNode_JumpDetails::FFlowNode_JumpDetails()
{
	bExcludeReferencedEvents = false;
}

void FFlowNode_JumpDetails::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	// For backward compatability, these localized texts are in FlowNode_CustomInputDetails, 
	//  not FlowNode_CustomNodeBase, so passing them in to a common function.
...
IDetailCategoryBuilder& FFlowNode_JumpDetails::CreateDetailCategory(IDetailLayoutBuilder& DetailLayout) const
{
	return DetailLayout.EditCategory("CustomInput", LOCTEXT("CustomInputCategory", "Custom Event"));
}

TArray<FName> FFlowNode_JumpDetails::BuildEventNames(const UFlowAsset& FlowAsset) const
{
	return FlowAsset.GetCustomInputs();
}
